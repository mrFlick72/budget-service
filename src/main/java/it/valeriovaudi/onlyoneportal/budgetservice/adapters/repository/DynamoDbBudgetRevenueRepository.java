...
public class DynamoDbBudgetRevenueRepository implements BudgetRevenueRepository {
    ...
    public List<BudgetRevenue> findByDateRange(String user, Date star, Date end) {
        ...
        return pks.stream()
                .flatMap(pk -> {
                    Map<String, AttributeValue> itemKeyCondition = yearlyItemKeyConditionFor(pk);
    ...
    private Map<String, AttributeValue> yearlyItemKeyConditionFor(String pk) {
    ...
    private Map<String, AttributeValue> putItemPayloadFor(BudgetRevenue budgetExpense) {
        ...
        payload.put("date", attributeValueFactory.stringAttributeFor(budgetExpense.getRegistrationDate().formattedDate()));
